...
<div class="page-header">
    <div class="d-flex justify-content-between align-items-center">
        <h1><i class="fas fa-eye me-2"></i>Chi Tiết Khuyến Mại</h1>
        <div>
                        ...
                        <table class="table table-borderless">
                            <tr>
                                ...
                                <td>
                                    ...
                                    <button class="btn btn-sm btn-outline-secondary ms-2" onclick="copyToClipboard('{{ $promotion->code }}')">
                            ...
                            <tr>
                                ...
                                <td>
                                    @if($promotion->valid_from)
                                        <span class="{{ $promotion->valid_from > now() ? 'text-warning' : 'text-success' }}">
                ...
                <div class="mb-4">
                    ...
                    @if($promotion->rooms->count() > 0)
                        ...
                        <div class="alert alert-success">
                            ...
                            Áp dụng cho <strong>{{ $promotion->rooms->count() }} phòng cụ thể</strong>
                        </div>
                        
                        @php
                            $roomsByType = $promotion->rooms->groupBy('roomType.name');
                        @endphp
                        
                        @foreach($roomsByType as $typeName => $rooms)
                            <div class="mb-3">
                                <h6 class="text-secondary">{{ $typeName }}</h6>
                                <div class="row">
                                            ...
                                            <div class="card bg-light border-success">
                                                <div class="card-body py-2">
                                                    <h6 class="card-title mb-1 text-success">
                                                        <i class="fas fa-bed"></i> {{ $room->room_number }}
                                                    </h6>
                                                    <small class="text-muted">{{ number_format($room->price, 0, ',', '.') }}đ/đêm</small>
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
                ...
                @if($promotion->usage_limit)
                    <div class="mt-3">
                        <div class="d-flex justify-content-between mb-1">
                            ...
                            <span class="small">{{ number_format(($promotion->used_count / $promotion->usage_limit) * 100, 1) }}%</span>
                        </div>
                        <div class="progress">
        ...
        <div class="card mt-3">
            ...
            <div class="card-body">
                <div class="d-grid gap-2">
                    <button class="btn btn-outline-primary btn-sm toggle-status" data-id="{{ $promotion->id }}" data-type="active">
                        <i class="fas fa-power-off"></i> 
                        {{ $promotion->is_active ? 'Tắt kích hoạt' : 'Kích hoạt' }}
                    </button>
                    
                    <button class="btn btn-outline-warning btn-sm toggle-status" data-id="{{ $promotion->id }}" data-type="featured">
                        <i class="fas fa-star"></i> 
                        {{ $promotion->is_featured ? 'Bỏ nổi bật' : 'Đặt nổi bật' }}
                    </button>
                    
                    <a href="{{ route('admin.promotions.edit', $promotion->id) }}" class="btn btn-outline-success btn-sm">
                        <i class="fas fa-edit"></i> Chỉnh sửa
                    </a>
                    ...
                    @php
                        ...
                        $canDelete = !$isExpired && !$isActive; // Chỉ được xóa khi "Sắp diễn ra"
                    @endphp
                    ...
                    @if($canDelete)
                        <form method="POST" action="{{ route('admin.promotions.destroy', $promotion->id) }}" class="d-inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-outline-danger btn-sm w-100" onclick="return confirm('Bạn có chắc muốn xóa khuyến mại này?')">
                                <i class="fas fa-trash"></i> Xóa khuyến mại
                            </button>
                        </form>
                    @else
                        ...
                        <small class="text-muted mt-1 d-block">
                            {{ $isExpired ? 'Không thể xóa khuyến mại đã kết thúc' : 'Không thể xóa khuyến mại đang hoạt động' }}
                        </small>
                    @endif
                </div>
            </div>
            ...
            <div class="card-body">
                <div class="promotion-preview">
                    ...
                    <div class="mt-3 text-center">
                        <a href="{{ route('promotions.show', $promotion->id) }}" target="_blank" class="btn btn-outline-primary btn-sm">
...
$(document).ready(function() {
    $('.toggle-status').click(function() {
        ...
        $.post('{{ url("admin/promotions") }}/' + id + '/toggle-status', {
            _token: '{{ csrf_token() }}',
            type: type
        })
